                ...
                <div class="input-group">
                    @if($single_compare == 0)
                        <input type="text"
                               ...
                               value="{{$start_field_value}}" class="text_range_start {{$class_name}}"
                               placeholder="{{$placeholder}}"
                               id="{{$search_field->sys_search_panel_details_id}}_start"
                                {{ $required == 1 ? 'required' : '' }}>
                    @endif
                    {{ Form::select($input_name.'_condition',
                            $compare_option,
                            $condition_field_value,
                            array('class'=>'text_range_condition form-control', 'data-id'=>$search_field->sys_search_panel_details_id, 'id'=>'range_condition_'.$search_field->sys_search_panel_details_id)) }}
                    <input type="text"
                           name="{{$input_name.'_end'}}"
                           value="{{$end_field_value}}" class="text_range_end {{$class_name}}"
                           placeholder="{{$placeholder}}"
                           id="{{$search_field->sys_search_panel_details_id}}_end"
                            {{ $required == 1 ? 'required' : '' }}>
                </div>
                <div class="help-block with-errors has-feedback"></div>
            </div>
...
<script>
    ...
    $(document).on('change', '.text_range_condition', function () {
        var id = $(this).data('id');
        checkRangeCondition(id);
    });
    function checkRangeCondition(id) {
        if($('#range_condition_'+id).val() != 'between'){
            $('#'+id+'_start').hide();
        }else{
            $('#'+id+'_start').show();
        }
