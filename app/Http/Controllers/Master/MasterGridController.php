            ...
            foreach ($table_data as $item) {
                ...
                foreach ($item as $name => $td_data) {
                    if ($name == $primary_key_field) {
                        ...
                        $row[] = $td_data == null || $td_data == '' ? 'N/A' : toDated($td_data);
                    } else if (strpos($name, 'price') || strpos($name, 'amount') || strpos($name, 'qty')) {
                        $row[] = "<h5 class='number-format text-right m-0'>" . datatable_moneyFormat($td_data) . "</h5>";
                    } else {
                        $row[] = $td_data;
                    }
...
/**===================================================================================================**/
    public function deleteRecord(Request $request){
        ...
        $record = DB::table($request_data['table_name'])->whereIn($request_data['primary_key_field'],$request_data['deleted_ids']);
        $msg = AuditTrailEvent::build($record)->delete();
